...
def compress_gif(input_img, quality, width, height, custom_size):
    ...
    for frame in range(0, img.n_frames):
        ...
        if custom_size:
            ...
            # Default scaling maintaining original aspect ratio by width
...
def images_to_gif(uploaded_files, duration, width, height, custom_size):
    frames = []
...
def video_to_gif(video_bytes, fps, width, height, custom_size):
    ...
    # FFmpeg crop logic for custom sizes
    if custom_size:
...
def show_size_guide(w, h):
    """Draws a visual colored block representing the selected canvas aspect ratio."""
    st.markdown(
        f"""
...
with tab2:
    ...
    if uploaded_imgs:
        ...
        preview_file = images_to_gif(uploaded_imgs, duration, img_width, img_height, custom_size_2)
        st.subheader("👀 Live Edit Preview:")
        st.image(preview_file)
        with open(preview_file, "rb") as file:
